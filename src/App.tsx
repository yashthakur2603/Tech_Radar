...
import { Bell, UploadCloud, Target, Sparkles, CheckCircle2, AlertCircle, FileText, Activity, DollarSign } from 'lucide-react';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Use 'click' instead of 'mousedown' for better consistency with button clicks
    ...
    const fetchNotificationsAndJobs = async () => {
      try {
        ...
        if (res.ok) {
          ...
          // Extract notifications array from the { notifications, jobs } response
        ...
        >
          ...
          {activeJobStatus === 'completed' && readyResult && !result && (
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center' }}>
              ...
              <p style={{ color: '#94A3B8', marginTop: 12, marginBottom: 20, fontSize: 16 }}>Your personalized tech radar is ready for review.</p>

              <div style={{ marginBottom: 30, padding: '12px 20px', background: 'rgba(99, 102, 241, 0.1)', border: '1px solid rgba(99, 102, 241, 0.2)', borderRadius: 12, display: 'flex', alignItems: 'center', gap: 10 }}>
                <Sparkles size={18} color="#A5B4FC" />
                <span style={{ color: '#A5B4FC', fontWeight: 600 }}>Click below to unveil the results</span>
              </div>
