...
import { AnalysisResult, Technology } from '../shared/types';
import { generatePDFReport } from '../utils/pdfExport';

interface CVRadarModuleProps {
  onAnalysisComplete: (result: AnalysisResult | null) => void;
  savedResult: AnalysisResult | null;
}

export default function CVRadarModule({ onAnalysisComplete, savedResult }: CVRadarModuleProps) {
  ...
  // Use the hoisted result if available, otherwise null
  ...
  const handleExportPDF = async () => {
    ...
    try {
      await generatePDFReport(result, targetRole);
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const newResult = typeof data === 'string' ? JSON.parse(data) : data;
  ...
  return (
    <div style={{ padding: '0 20px 40px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(350px, 1fr) 1.5fr', gap: 30, alignItems: 'start' }}>
        ...
        {/* INPUT FORM */}
        <div className="glass-panel" style={{ padding: 24, height: 'max-content' }}>
